...
function setup() {
  ...
  let uniqueCategories = table
    .getColumn("TypeCategory")
    .filter((v, i, a) => a.indexOf(v) === i && v !== ""); //recupera tutti valori presenti in colonna csv TypeCategory
  ...
  for (let i = 0; i < uniqueCategories.length; i++) {
    //itera su ogni elemento nell'array uniqueCategories
...
function calculateMapDimensions() {
  ...
  legendCategoryY =
    legendY_Elevation + elevationRectHeight + legendCategoryMargin; //posizione Y = Y di Elevation + Altezza Elevation + Margine
...
function draw() {
  ...
  for (let i = 0; i < table.getRowCount(); i++) {
    ...
    if (activeLegend === "Elevation") {
      //se attiva legenda Elevation, usa logica Elevation
    ...
    } else if (activeLegend === "Type category") {
      //se attiva legenda Type Category, usa colore mappato

      //colora in base a type category
      volcanoColor = categoryColors[typeCategory]
        ? color(categoryColors[typeCategory])
  ...
  if (hoveredVolcano) {
    const tooltipContent = [
      //testo tooltip, mostra nome, country, altitude, type, type cateogry, status, last known eruption
...
function drawTooltip(x, y, content) {
  ...
  for (const item of content) {
    ...
    if (currentWidth > panelWidth) {
      //blocco verifica se larghezza testo corrente > larghezza massima finora trovata (panelWidth)
  ...
  for (const item of content) {
    ...
    if (item.bold) {
      //se array const tooltipContent[] ha bold:true per quell'item
...
function drawUIElements() {
  ...
  if (activeLegend === "Elevation") {
    //applica stroke solo se elevatioe è legenda attiva
...
function drawTypeCategoryLegend() {
  ...
  if (activeLegend === "Type category") {
    //applica stroke solo se type category è la legenda attiva
...
function mouseClicked() {
  ...
  if (
    mouseX >= elevationRectX &&
    mouseX <= elevationRectX + elevationRectWidth &&
    mouseY >= elevationRectY &&
    mouseY <= elevationRectY + elevationRectHeight
  ) {
    activeLegend = "Elevation"; //imposta elevation attiva quando si apre sito
  }
  //controlla se click avvenuto su legenda type category
  else if (
    mouseX >= categoryRectX &&
    mouseX <= categoryRectX + categoryRectW &&
    mouseY >= categoryRectY &&
    mouseY <= categoryRectY + categoryRectH
  ) {
